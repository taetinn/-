...
@Log
public class FileModifyProAction implements Action {

	@Override
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		log.info("FileModifyProAction..............");
		...
		for (FileItem item : list) {
			if (item.isFormField()) {
				if (item.getFieldName().equals("name")) {
					...
					delUuidList.add(uuid); // 삭제할 첨부파일 정보를 리스트에 추가
				}
			}
		} // for
		...
		if (delUuidList.size() > 0) {
			attachDao.deleteAttachfilesByUuids(delUuidList);
		}
