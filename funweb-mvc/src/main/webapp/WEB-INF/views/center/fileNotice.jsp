...
<div id="wrap">
	...
	<div id="table_search">
		...
		<c:if test="${ not empty id }"><%-- ${ not empty sessionScope.id } --%>
			<input type="button" value="글쓰기" class="btn" onclick="location.href='fileWriteForm.do?pageNum=${ pageNum }'">
		</c:if>
	</div>
	...
	<div id="page_control">
		<c:if test="${ pageDto.totalCount gt 0 }">
			...
			<c:forEach var="i" begin="${ pageDto.startPage }" end="${ pageDto.endPage }" step="1">
				<a href="/fileNotice.do?pageNum=${ i }&category=${ pageDto.category }&search=${ pageDto.search }">
				<c:choose>
